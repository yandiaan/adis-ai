...
const DIMENSIONS: { value: ImageDimension; label: string }[] = [
  { value: 'square-1024', label: '1024×1024 (Kotak)' },
  { value: 'portrait-768x1024', label: '768×1024 (Potret)' },
  { value: 'landscape-1024x768', label: '1024×768 (Lanskap)' },
            ...
            <div className="inline-flex items-center gap-2">
              {config.mode === 'text2img' ? (
                ...
                <>
                  ...
                  <ArrowRight size={14} className="text-white/40" />
                  <Image size={14} className="text-white/70" />
                  <span>Gambar ke Gambar</span>
                </>
              )}
            </div>
            <div className="text-[11px] text-white/30 mt-1">
              Terdeteksi otomatis dari koneksi
            </div>
          </div>
        </div>
      ...
      <DrawerSection title="Lanjutan" defaultCollapsed accent="advanced">
        ...
        <div>
          ...
          <p className="text-[11px] text-white/30 mb-2">
            AI akan memperluas prompt-mu secara otomatis untuk hasil lebih detail.
          </p>
        ...
        <div>
          ...
          <p className="text-[11px] text-white/30 mb-2">
            Isi angka untuk mendapatkan hasil yang sama setiap run. Kosongkan untuk hasil acak.
          </p>
          <input
            type="number"
            value={config.seed ?? ''}
            onChange={(e) =>
              updateConfig({ seed: e.target.value ? Number(e.target.value) : null })
            }
            placeholder="Acak (default)"
