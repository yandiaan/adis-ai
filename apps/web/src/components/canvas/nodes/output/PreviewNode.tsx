...
export function PreviewNode({ data, selected }: NodeProps<Node<PreviewData>>) {
  ...
  const boxW = 64;
  const boxH = Math.round((boxW * ah) / aw);

  return (
    <CompactNode nodeType="preview" icon="" title={data.label} selected={selected}>
      <div className="flex items-start gap-2">
        ...
        >
          {resolvedImageUrl ? (
            <img src={resolvedImageUrl} alt="Preview" className="w-full h-full object-cover" />
          ) : resolvedVideoUrl ? (
            <video
